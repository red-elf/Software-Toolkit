...
if sh "$SCRIPT_GET_DOCKER" true; then
  ...
  if [ "$CONTAINER_STATUS" = "running" ]; then
    ...
    if [ "$CONTAINER_STATUS" = "exited" ]; then
      ...
      REPLACE "$PROTO_CONTENT" "{{SERVICE.SONAR_QUBE.PORTS.PORT_EXPOSED}}" "$PARAM_SONARQUBE_PORT" "$FILE_DOCKER_COMPOSE_PATH"
      REPLACE "$PROTO_CONTENT" "{{SERVICE.DATABASE.USER}}"                 "$DB_USER"              "$FILE_DOCKER_COMPOSE_PATH"
      REPLACE "$PROTO_CONTENT" "{{SERVICE.DATABASE.PASSWORD}}"             "$DB_PASSWORD"          "$FILE_DOCKER_COMPOSE_PATH"
